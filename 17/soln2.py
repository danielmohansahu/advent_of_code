        ...
        for j in range(shape[1]):
            for k in range(shape[2]):
                for l in range(shape[3]):
                    ...
                    ws = (max(l-1,0),min(l+2,shape[3]))
                    neighbors = padded[xs[0]:xs[1],ys[0]:ys[1],zs[0]:zs[1],ws[0]:ws[1]]

                    # count of positive neighbors is count MINUS our value
                    count = neighbors.sum() - (1 if padded[i,j,k,l] else 0)

                    # apply our rules
                    if padded[i,j,k,l] and (count not in (2,3)):
                        result[i,j,k,l] = False
                    elif not padded[i,j,k,l] and (count == 3):
                        result[i,j,k,l] = True
...
if __name__ == "__main__":
    ...
    # add the Z and W axes
    array.resize(*array.shape,1,1)
