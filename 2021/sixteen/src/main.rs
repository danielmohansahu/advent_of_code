...
fn decode(bits: &[u8], count: &mut u64) -> usize {
    ...
    if type_ == 4 {
        ...
        match op_type {
            0 => {
                ...
                loop {
                    cur += decode(&bits[cur..end], count);
                    if cur == end {
                        break;
                    }
                }

                // update out estimate of packet length
                packet_length = cur;
            },
            1 => {
